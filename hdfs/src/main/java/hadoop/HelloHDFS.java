...
public class HelloHDFS {
	public static void main(String[] args) throws Exception {
		 ...
		 */
		// 传输文件
		String[] fileName = getFileName("/media/segust/Application/04");
		for (String filename : fileName) {
			System.out.println(filename);
			FSDataOutputStream out = fileSystem.create(new Path("/user/hive/" + filename), true);
			FileInputStream fis = new FileInputStream("/media/segust/Application/04/" + filename);
